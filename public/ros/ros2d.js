 ...
 *  图像地图是一个PNG图像，缩放后符合OccupancyGrid的尺寸。
 * @constructor
 ...
 * @constructor
 * @param options - object with following keys:
 *   * message - the occupancy grid message 占用网格消息
 */
ROS2D.OccupancyGrid = function (options) {
  options = options || {};
  var message = options.message;

  // internal drawing canvas 内部绘图画布
  var canvas = document.createElement('canvas');
  var context = canvas.getContext('2d');

  // save the metadata we need 保存我们需要的元数据
  this.pose = new ROSLIB.Pose({
    position: message.info.origin.position, // position 位置
    orientation: message.info.origin.orientation  // orientation 方向
  });
  ...
  for (var row = 0; row < this.height; row++) {
    for (var col = 0; col < this.width; col++) {
      // determine the index into the map data  确定地图数据的索引
      var mapI = col + ((this.height - row - 1) * this.width);
      // determine the value  确定值
 ...
 * 监听给定占位网格主题的地图。
 * Emits the following events: 发出以下事件：
 *   * 'change' - there was an update or change in the map
 *   “更改”-地图中有更新或更改
 * @constructor
 * @param options - object with following keys:
...
ROS2D.OccupancyGridClient = function (options) {
  ...
  this.rootObject = options.rootObject || new createjs.Container();
  // current grid that is displayed   显示的当前网格
  ...
  // subscribe to the topic  订阅该主题
  var rosTopic = new ROSLIB.Topic({
    ros: ros,
    name: topic,
    messageType: 'nav_msgs/OccupancyGrid',//'std_msgs/String' base64
    compression: 'png'
  });


  rosTopic.subscribe(function (message) {
    // check for an old map  检查旧地图
    var index = null;
    if (that.currentGrid) {
      index = that.rootObject.getChildIndex(that.currentGrid);
      that.rootObject.removeChild(that.currentGrid);
    }
    console.log('rosTopic.subscribe', message);
    ...
    // check if we should unsubscribe 检查我们是否应该取消订阅
    if (!that.continuous) {
      // rosTopic.unsubscribe();
    }
  });
...
ROS2D.OccupancyGridSrvClient = function (options) {
  ...
  // Setting up to the service 设置服务
...
ROS2D.ArrowShape = function (options) {
  ...
  // check if we are pulsing 检查我们是否有脉搏
  if (pulse) {
    // have the model "pulse" 具有模型“脉冲”
    var growCount = 0;
    var growing = true;
...
/**
 * A Grid object draw in map. 在地图中绘制网格对象。
 *
 * @constructor
 * @param options - object with following keys:
