  ...
  methods: {
    ...
    loadSTL() {
      ...
      PCDLoader1.load('/three/GlobalMap.pcd', (points) => {
        ...
        points.position.set(7, -24, 18);//旋转模型，可调
      ...
      loader.load(`xun.STL`, (geometry) => {
        const material = new THREE.MeshLambertMaterial({ color: 0xf4e56d9 });
        base = new THREE.Mesh(geometry, material);
        // console.log(mash);

        base.scale.set(0.0015, 0.0015, 0.0015)
        base.rotation.set(0.5 * Math.PI, 0, 0)
    ...
    render() {
      ...
      this.camera.lookAt(transforms.position.x, transforms.position.y, 0);
      this.camera.position.x = transforms.position.x;
        this.camera.position.y = transforms.position.y+20;
      this.camera.position.z = 20;

      var that = this

      function poselist(link, item) {
        var { orientation, position } = item;
        // console.log(link);
        if (!link) return;

        link.position.set(position.x, position.y, position.z);
        // link.quaternion.set(orientation.x, orientation.y, orientation.z, orientation.w);
        link.rotation.set(orientation.x, orientation.y, orientation.z, orientation.w);
        // link.rotation.set(0.5 * Math.PI,Math.PI, 0)
      }
      if (transforms) poselist(base, transforms);
