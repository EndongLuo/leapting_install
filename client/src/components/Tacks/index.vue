<template>
  <div>
    <div class="tacks" @click="isTask" v-if="taskState.id">
      <span><b>{{ $t('task.task') }}：</b>{{ taskState.task_type == 1 ? `${$t('install.sai')}` : `${$t('install.fai')}`
        }}(<span style="font-weight: 700;"><span v-if="taskState.task_status == 3">{{ $t('task.completed') }}</span>
      ...
      <span><b>{{ $t('task.progress') }}：</b>{{ (taskState.done_num / taskState.task_num) * 100 }}%
